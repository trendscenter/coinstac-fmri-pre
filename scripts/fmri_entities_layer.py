...
## 1 FSL roi extraction to remove specified number of volumes at the beginning of the series ##
class Fslroi:
    def __init__(self, nifti_file, **template_dict):
        self.node = pe.Node(interface=fsl.ExtractROI(), name='fslroi')
        self.node.inputs.in_file=nifti_file
        self.node.inputs.z_min = template_dict['num_vols_to_remove']
        self.node.inputs.z_size = num_vols
...
## 4 Slicetiming Node and settings ##
class Slicetiming:
    def __init__(self, **template_dict):
        self.node = pe.Node(interface=spm.SliceTiming(), name='slicetiming')
        self.node.inputs.paths = template_dict['spm_path']

## 5 Normalize Node and settings ##
class Normalize:
    def __init__(self, **template_dict):
...
## 7 Datsink Node that collects segmented, smoothed files and writes to temp_write_dir ##
class Datasink:
    def __init__(self):
